...
public:
    ...
    {
        return Validity() != ChainValidity::INVALID;
    }
...
public:
    ...
    Chainstate& CurrentChainstate() const EXCLUSIVE_LOCKS_REQUIRED(GetMutex())
    {
        for (auto& cs : m_chainstates) {
            if (cs && cs->IsValid() && !cs->m_target_blockhash) return *cs;
        }
        abort();
    }
    ...
    {
        for (auto& cs : m_chainstates) {
            if (cs && cs->IsValid() && cs->m_target_blockhash && !cs->m_target_utxohash) return cs.get();
        }
        return nullptr;
    }
